...
(async () => {
  const browser = await puppeteer.launch({ args: [...chromium.args, '--no-sandbox', '--disable-gpu', '--single-process', '--no-zygote', '--disable-dev-shm-usage'],
    ...
    defaultViewport: { width: Number(process.env.W || 1280), height: Number(process.env.H || 800) } });
  ...
  const info = await page.evaluate(() => ({
    ...
    quickbar: [...document.querySelectorAll('.quickbar .btn')].map((b) => b.textContent.trim()),
  }));
